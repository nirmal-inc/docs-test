...
For the original approach using a central ledger, see [Inter\_energy\_retailer\_P2P\_trading\_draft.md](Inter_energy_retailer_P2P_trading_draft.md).
...
| trade qty                    |       Yes      |    Yes    |
| delivery start\_time         |       Yes      |    Yes    |
| delivery end\_time           |       Yes      |    Yes    |
| allocated qty                |       Yes      |    Yes    |
| counterparty's allocation    |       Yes      |    Yes    |
| settled\_qty                 |       Yes      |    Yes    |
...
For utilities that choose to operate via their LP — or where network policy mandates this — the [Overall Process Flow](Inter_energy_retailer_P2P_trading.md#overall-process-flow) sequence diagram above applies **unchanged**. The only substitution is at the participant level:
...
The block diagram below summarises the LP-mediated topology — sourced from _Final P2P Ledger and Allocation Architecture_ (Pramod Varma, 15 April 2025).
